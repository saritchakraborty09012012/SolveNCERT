-- ═══════════════════════════════════════════════════════════════════════════
-- v17 ALL-IN-ONE — Collaboration + Chat + OTP + Queries
-- RUN THIS ONCE in Supabase SQL Editor.
-- CLOSE all localhost tabs FIRST to avoid deadlocks.
-- If any statement errors because table/column already exists, that's fine —
-- the IF NOT EXISTS / IF EXISTS guards handle it.
-- ═══════════════════════════════════════════════════════════════════════════

-- ── 0. Columns on existing tables ───────────────────────────────────────────
...
-- ── 1. log_page_visit with attribution ──────────────────────────────────────
...
-- 2k. queries (for header query button)
...
-- ── 6. DROP old policies that cause 500, then CREATE clean ones ────────────
-- First: drop the problematic "Collab members view host profile" that kills login
DROP POLICY IF EXISTS "Collab members view host profile" ON public.profiles;

-- profiles: owner policies only (safe, no 500)
...
-- Invitees can see host profile (uses SECURITY DEFINER helper, no recursion)
...
CREATE POLICY "Members view collab session" ON public.collab_sessions
  FOR SELECT USING (
    auth.uid() = host_id OR public.collab_is_member_of_host(host_id, auth.uid())
  );
...
CREATE POLICY "Members create groups" ON public.chat_groups
  FOR INSERT WITH CHECK (created_by = auth.uid() AND public.collab_is_member_of_host(host_id, auth.uid()));
...
CREATE POLICY "Members view memberships" ON public.chat_group_members
  FOR SELECT USING (
    user_id = auth.uid() OR public.collab_is_member_of_host(
      (SELECT host_id FROM public.chat_groups WHERE id = chat_group_members.group_id), auth.uid()
    )
  );
...
CREATE POLICY "Members add memberships" ON public.chat_group_members
  FOR INSERT WITH CHECK (
    public.collab_is_member_of_host(
      (SELECT host_id FROM public.chat_groups WHERE id = chat_group_members.group_id), auth.uid()
    )
    AND (user_id = auth.uid() OR 
      (SELECT created_by FROM public.chat_groups WHERE id = chat_group_members.group_id) = auth.uid() OR
      (SELECT host_id FROM public.chat_groups WHERE id = chat_group_members.group_id) = auth.uid())
  );
...
CREATE POLICY "Leave or host removes" ON public.chat_group_members
  FOR DELETE USING (
    user_id = auth.uid() OR (
      SELECT host_id FROM public.chat_groups WHERE id = chat_group_members.group_id
    ) = auth.uid()
  );
...
CREATE POLICY "Members view messages" ON public.chat_messages
  FOR SELECT USING (
    public.collab_is_member_of_host(
      (SELECT host_id FROM public.chat_groups WHERE id = chat_messages.group_id), auth.uid()
    )
    AND NOT EXISTS (
...
CREATE POLICY "Members send messages" ON public.chat_messages
  FOR INSERT WITH CHECK (
    sender_id = auth.uid() AND public.collab_is_member_of_host(
      (SELECT host_id FROM public.chat_groups WHERE id = chat_messages.group_id), auth.uid()
    )
  );
...
CREATE POLICY "Sender or host updates messages" ON public.chat_messages
  FOR UPDATE USING (
    sender_id = auth.uid() OR (
      SELECT host_id FROM public.chat_groups WHERE id = chat_messages.group_id
    ) = auth.uid()
  );
...
CREATE POLICY "Members mark reads" ON public.chat_message_reads
  FOR INSERT WITH CHECK (
    user_id = auth.uid() AND public.collab_is_member_of_host(
      (SELECT host_id FROM public.chat_groups WHERE id = (
        SELECT group_id FROM public.chat_messages WHERE id = chat_message_reads.message_id
      )), auth.uid()
    )
  );
...
CREATE POLICY "Author or host removes strokes" ON public.collab_board_strokes
  FOR DELETE USING (
    ...
    (SELECT host_id FROM public.collab_sessions WHERE id = collab_board_strokes.session_id) = auth.uid()
  );
...
-- queries: anyone can insert, only service_role reads
...
-- ── 7. Collab access to host data (view + add + update, never delete by invitee) ──
DO $$
DECLARE t text;
BEGIN
...
-- ═══════════════════════════════════════════════════════════════════════════
-- DONE. Refresh the site. All tables + policies + functions ready.
-- ═══════════════════════════════════════════════════════════════════════════
